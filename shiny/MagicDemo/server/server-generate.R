  ...
  withProgress(value = 0, 
               {
                 ...
                 tg <- "human"   #### needs to be fixed
                 generate_output <- NULL
                 generate_running <- TRUE
                 generate_cmd <- paste("Rscript")
                 args <- paste(generate_file,
                               "-i", paste("data/joined_scores/", input$fileJoined, sep=""),
                               "-o", paste("data/out", "/model_output", sep=""),
                               "-m", input$metric,
...
observeEvent(input$plotModelsButton, {
  ...
  modelTable <- load_data(paste("data/out", "/model_output/summary_models.tsv", sep=""))
  ...
  if (!is.null(input$validationSet$datapath)) {
    ...
    validation[[input$targetFeature]] <- sub("MAE", "MAE", validation[[input$targetFeature]])
  } else {
    training_set <- load_data(input$fileJoined$datapath)
    if (input$tg == "human"){
      ...
      validation[["status"]] <- sub("MAE", "MAE", validation[["status"]])
  ...
  #### get names of models
  model_names <- list.files(paste("data/out", "/model_output", sep=""), pattern = "*_model.rds")
  model_list <- list()
  # assign models to r environment variables
  for (i in 1:length(model_names)){
    name_string <- as.character(model_names[i])
    assign(name_string, readRDS(paste("data/out", "/model_output/", model_names[i], sep = "")))
  }
