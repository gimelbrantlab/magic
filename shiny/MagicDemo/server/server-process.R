...
observeEvent(input$processDataButton, 
  ...
  if (!is.null(input$fileInput)) {
    ...
    assembly <- "hg19"
    if (input$fileInput=="input_mouse.txt") assembly <- "mm10"
    args <- paste(process_file,
                  "-i", paste("data/input/", input$fileInput, sep = ""),
                  "-o", paste("data/out"),
  ...
  if (file.exists(paste("data/out", "/joined_scores_percentile.txt", sep = ""))){
    joined_scores_percentile <- load_data(paste("data/out", "/joined_scores_percentile.txt", sep = ""))
    joined_scores_norm <- load_data(paste("data/out", "/joined_scores_norm.txt", sep = ""))
      ...
      input_files <- list.files("data/out", pattern = "_input", full.names = TRUE)
